...
public class DBWorker {
    public static final String PATH_TO_DB_FILE="src/main/data/database/BeautyShop.db";
    ...
    public static void createDB(){
        try {
            ...
            statement.execute("CREATE TABLE if not exists 'rewards' ( 'transfersPoint' int NOT NULL, 'discountPoint' int NOT NULL);");
    ...
    public static void addPerformedWork(Work p){
        try {
            PreparedStatement S = connection.prepareStatement("INSERT INTO performedWork (" +
                    "'name','price', 'point' ,'income' ," +
                    "'id_clients','surname_clients','name_clients','middle_clients','id_employee','surname_employee'," +
                    "'name_employee','middle_employee'," +
                    "'date','time','comment') VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    public static void addRecord(Record p){
        try {
            PreparedStatement S = connection.prepareStatement(
                    "INSERT INTO record (" +
                    "'name','price','id_clients','surname_clients'," +
                    "'name_clients','middle_clients','id_employee','surname_employee'," +
                    "'name_employee','middle_employee'," +
                    "'date','time','comment') VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
            S.setObject(1,p.getServices().getName());
            S.setObject(2,p.getServices().getPrice());
    ...
    public static void changeClients(int id,Clients c){
        try {
            ...
            for (Record r:rec
                ...
                if(r.getClients().getId()==id){
                    PreparedStatement S2 = connection.prepareStatement("UPDATE record SET " +
                            "'surname_clients'=?,'name_clients'=?," +
                            "'middle_clients'=?WHERE id_record =? ");
            ...
            PreparedStatement S3 = connection.prepareStatement("UPDATE performedWork SET " +
                    "'surname_clients'=?,'name_clients'=?,'middle_clients'=?WHERE id_clients =? ");
    ...
    public static void changeEmployee(int id,Employee c){
        try {
            ...
            S.execute();
            S.close();
            PreparedStatement S2 = connection.prepareStatement("UPDATE record SET 'surname_employee'=?,'name_employee'=?,'middle_employee'=?WHERE id_employee =? ");
            S2.setObject(1,c.getSurname());
            S2.setObject(2,c.getName());
            S2.setObject(3,c.getMiddle());
            S2.setObject(4,id);
            ...
            S2.close();
            PreparedStatement S3 = connection.prepareStatement("UPDATE performedWork SET " +
                    "'surname_employee'=?,'name_employee'=?,'middle_employee'=?WHERE id_employee =? ");
            S3.setObject(1,c.getSurname());
            S3.setObject(2,c.getName());
            S3.setObject(3,c.getMiddle());
            S3.setObject(4,id);
    ...
    public static void changeRecord(int id,Record p){
        try {
            PreparedStatement S = connection.prepareStatement("UPDATE record SET 'name'=?,'price'=?,'id_clients'=?," +
                    "'surname_clients'=?,'name_clients'=?,'middle_clients'=?,'id_employee'=?,'surname_employee'=?," +
                    "'name_employee'=?,'middle_employee'=?,'date'=?,'time'=?,'comment'=?WHERE id_record =? ");
            S.setObject(1,p.getServices().getName());
            S.setObject(2,p.getServices().getPrice());
            S.setObject(3,p.getClients().getId());
            S.setObject(4,p.getClients().getSurname());
