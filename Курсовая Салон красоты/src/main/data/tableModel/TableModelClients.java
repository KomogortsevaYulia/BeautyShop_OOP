        ...
        if(res) {
            ...
            for (Clients c : data
                ...
                if (c.getSurname().equals(p.getSurname()) & c.getName().equals(p.getName()) && c.getMiddle().equals(p.getMiddle())) {
                    Log.addLog("Таблица \"Клиенты\" " + " Добавили клиента: ID:" + c.getId() +
    ...
    public boolean changeRow(int id,Clients p){
        ...
        if(res) {
            ...
            for (Clients c : data
                ...
                if (c.getId() == id) {
                    Log.addLog("Таблица \"Клиенты\" " + " Изменили данные клиента: ID:" + c.getId() +
    ...
    public void deleteRow(int[] id){
        for (int i = 0; i < id.length; i++) {
            for (Clients c:data
                ...
                if(id[i]==c.getId()){
                    Log.addLog("Таблица \"Клиенты\" "+" Удалили данные клиента: ID:"+c.getId()+
                            " ФИО: "+c.getSurname()+" "+c.getName()+" "+c.getMiddle()+
                            " Телефон: "+c.getPhone()+" День рождения: "+c.getBirthdate()+ " Почта: "+c.getEmail()
                            +" Баллы: "+c.getPoint());
                }
